...
namespace KartGame.Track
{
    ...
    public class TimeDisplay : MonoBehaviour
    {
        ...
        void UpdateLap()
        {
            ...
            if (convertTimeStringToSeconds(GameSaveManager.Instance.Load(ScoreType.BestLap, 0)) > finishedLapTimes[getBestLap()]) {
                NotificationHUDManager notificationHUDManager = FindObjectOfType<NotificationHUDManager>();
                notificationHUDManager.CreateNotification("New Record!");
            }
            GameSaveManager.Instance.SaveState(ScoreType.BestLap,DisplaySessionBestLapTime());
        ...
        public float convertTimeStringToSeconds(string timeString) {
            ...
            float fraction = float.Parse(timeParts[2]);

            float totalSeconds = minutes * 60 + seconds + fraction / 100; 
